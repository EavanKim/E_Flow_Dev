...
void Graphic_Vulkan::initVulkan()
{
	...
	createCommandPool();
		...
		size_t Count = 0;
		for (GV_Obj* Obj : m_Object)
		{
			switch (Obj->GetBufferType())
			{
			case BUFFER_TYPE::SPRITE_BUFFER:
				{
					Sprite_Obj* Spr = dynamic_cast<Sprite_Obj*>(Obj);
					createUniformBuffers(*Spr);
					createDescriptorSets(Count, *Spr);
					++Count;
				}
				break;
			case BUFFER_TYPE::RECTANGLE_BUFFER:
...
void Graphic_Vulkan::readScene()
{
	...
	CreateSpriteObject("textures/EavanKimTrash.png");
...
void Graphic_Vulkan::cleanup()
{
	...
	for (GV_Obj* Obj : m_Object)
	{
		if (BUFFER_TYPE::SPRITE_BUFFER == Obj->GetBufferType())
		{
			Sprite_Obj* Spr = dynamic_cast<Sprite_Obj*>(Obj);
			Spr->DestroyInstance(m_Device);
			delete Spr;
		}
		else
		{
			Obj->DestroyInstance(m_Device);
			delete Obj;
		}
	}
	m_Object.clear();

	for (std::pair<int, GV_Camera*> Camera : m_Camera)
	{
		Camera.second->DestroyInstance(m_Device);
...
void Graphic_Vulkan::CheckSuccessInRuntime(VkResult _result, char* _comment)
{
...
void Graphic_Vulkan::CheckSuccessInRuntime(VkResult _result, const char* _comment)
{
...
VkApplicationInfo Graphic_Vulkan::GetApplicationInfo(char* _applicationName, char* _enginName, int _appMajorVersion, int _appMinorVersion, int _appPatchVersion, int _engineMajorVersion, int _engineMinorVersion, int _enginePatchVersion)
{
...
VkApplicationInfo Graphic_Vulkan::GetApplicationInfo(const char* _applicationName, const char* _enginName, int _appMajorVersion, int _appMinorVersion, int _appPatchVersion, int _engineMajorVersion, int _engineMinorVersion, int _enginePatchVersion)
{
	VkApplicationInfo VkInfo = {};
...
void Graphic_Vulkan::CreateBufferObject(BUFFER_TYPE _Type)
{
	GV_Obj* BufferObject = new GV_Obj(BUFFER_TYPE::RECTANGLE_BUFFER, IMAGE_TYPE::IMAGE_END);

	BufferObject->GetVertex(createVertexBuffer(rectangle));
	BufferObject->GetIndex(createIndexBuffer(rectIndices));

	m_Object.push_back(BufferObject);
}

void Graphic_Vulkan::CreateCameraObject(CAMERA_TYPE _Type, uint32_t _width, uint32_t _height)
{
	m_Camera.insert(std::pair(m_Camera.size(), new Static_Camera(_width, _height, glm::vec3(0.f))));
}

void Graphic_Vulkan::CreateCameraObject(CAMERA_TYPE _Type, uint32_t _width, uint32_t _height, GV_Obj* _Player)
{
	m_Camera.insert(std::pair(m_Camera.size(), new Static_Camera(_width, _height, glm::vec3(0.f), _Player)));
}

void Graphic_Vulkan::CreateSpriteObject(std::string _Path)
{
	Sprite_Obj* Sprite = new Sprite_Obj(LoadingImage(_Path));

	Sprite->GetVertex(createVertexBuffer(rectangle));
	Sprite->GetIndex(createIndexBuffer(rectIndices));

	m_Object.push_back(Sprite);
}

GV_Image Graphic_Vulkan::LoadingImage(std::string _Path)
{
	GV_Image Image = {};
	...
	createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, Image.m_Image, Image.m_ImageMemory);

	transitionImageLayout(Image.m_Image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	copyBufferToImage(stagingBuffer, Image.m_Image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
	transitionImageLayout(Image.m_Image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

	vkDestroyBuffer(m_Device, stagingBuffer, nullptr);
	vkFreeMemory(m_Device, stagingBufferMemory, nullptr);

	Image.m_ImageView = createImageView(Image.m_Image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
	CreateTextureSampler(Image.m_Sampler);

	return Image;
}

void Graphic_Vulkan::Key_callback(GLFWwindow* _window, int _key, int _scancode, int _action, int _mods)
{
	switch (_action)
	{
	case GLFW_PRESS:
		...
		case GLFW_KEY_ESCAPE:
			Graphic_Vulkan::GetInstance()->WindowQuit();
			break;
		case GLFW_KEY_W:
			Graphic_Vulkan::GetInstance()->KeyDoing3(true);
			break;
		case GLFW_KEY_S:
			Graphic_Vulkan::GetInstance()->KeyDoing3(false);
			break;
		case GLFW_KEY_D:
			Graphic_Vulkan::GetInstance()->KeyDoing2(true);
			break;
		case GLFW_KEY_A:
			Graphic_Vulkan::GetInstance()->KeyDoing2(false);
			break;
		}
		break;
	case GLFW_REPEAT:
...
void Graphic_Vulkan::WindowQuit()
{
	glfwSetWindowShouldClose(m_Window, GLFW_TRUE);
}

void Graphic_Vulkan::KeyDoing(bool _leftright)
{
	if(_leftright)
		m_Object[0]->SetAngle(0.1f, 0, 2.0f, 0);
	else
		m_Object[0]->SetAngle(0.1f, 0, -2.0f, 0);

	m_Camera[0]->Update(m_Object[0]->GetPosVector());
}

void Graphic_Vulkan::KeyDoing1(bool _updown)
{
	if (_updown)
		m_Object[0]->SetAngle(0.1f, 0, 2.0f, 0);
	else
...
void Graphic_Vulkan::KeyDoing2(bool _leftright)
{
	if (_leftright)
		m_Object[0]->Move(0.1f, 0, 0);
	else
		m_Object[0]->Move(-0.1f, 0, 0);
...
void Graphic_Vulkan::KeyDoing3(bool _updown)
{
	if (_updown)
		m_Object[0]->Move(0, 0.1f, 0);
	else
		m_Object[0]->Move(0, -0.1f, 0);
...
void Graphic_Vulkan::createmultipassGraphicsPipeline()
{
	//char curDir[1000];
	//_getcwd(curDir, 1000);
	//printf(curDir);
	...
	std::vector<char> fragShaderCode = readFile("shaders/frag2.spv");
...
//void Graphic_Vulkan::createCommandBuffers(std::vector<VkImage> _SwapChainFramebuffers, VkExtent2D _SwapChainExtent, std::vector<VkCommandBuffer> _CommandBuffers, VkRenderPass _RenderPass, VkPipeline _GraphicsPipeline)
//{
...
//
//	if (vkAllocateCommandBuffers(m_Device, &allocInfo, _CommandBuffers.data()) != VK_SUCCESS)
//		throw std::runtime_error("failed to allocate command buffers!");
//
//	for (size_t Count = 0; _CommandBuffers.size() > Count; ++Count)
//	{
//		VkCommandBufferBeginInfo beginInfo = {};
//		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
//		//beginInfo.flags = 0; // Optional
//		//beginInfo.pInheritanceInfo = nullptr; // Optional
//
//		if (vkBeginCommandBuffer(_CommandBuffers[Count], &beginInfo) != VK_SUCCESS)
//			throw std::runtime_error("failed to begin recoding command buffer!");
//
//		VkRenderPassBeginInfo renderPassInfo = {};
//		renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
//		renderPassInfo.renderPass = _RenderPass;
//		renderPassInfo.framebuffer = _SwapChainFramebuffers[Count];
//
//		renderPassInfo.renderArea.offset = { 0, 0 };
...
//		///
//
//		vkCmdBeginRenderPass(_CommandBuffers[Count], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
//
//		vkCmdBindPipeline(_CommandBuffers[Count], VK_PIPELINE_BIND_POINT_GRAPHICS, _GraphicsPipeline);
//
//		VkBuffer vertexBuffers[] = { m_VertexBuffer };
//		VkDeviceSize offsets[] = { 0 };
//		vkCmdBindVertexBuffers(_CommandBuffers[Count], 0, 1, vertexBuffers, offsets);
//
//		///
//		//Index Buffer
//		vkCmdBindIndexBuffer(_CommandBuffers[Count], m_IndexBuffer, 0, VK_INDEX_TYPE_UINT16);
//
//		///
//		//uniform buffer
//		vkCmdBindDescriptorSets(_CommandBuffers[Count], VK_PIPELINE_BIND_POINT_GRAPHICS, m_PipelineLayout, 0, 1, &m_DescriptorSets[Count], 0, nullptr);
//		///
//
//		vkCmdDrawIndexed(_CommandBuffers[Count], static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
//		//vkCmdDraw(_CommandBuffers[Count], static_cast<uint32_t>(vertices.size()), 1, 0, 0);
//		///
//
//
//		vkCmdEndRenderPass(_CommandBuffers[Count]);
//
//		if (vkEndCommandBuffer(_CommandBuffers[Count]) != VK_SUCCESS)
//			throw std::runtime_error("failed to record command buffer!");
//	}
//}

void Graphic_Vulkan::createCustomCommandBuffers()
{
	m_CommandBuffers.resize(m_SwapChainFramebuffers.size());
	...
	for (size_t Count = 0; m_CommandBuffers.size() > Count; ++Count)
	{
		...
		for(GV_Obj* Obj : m_Object)
		{
			VkBuffer vertexBuffers[] = { Obj->GetVertex().m_Buffer };
			VkDeviceSize offsets[] = { 0 };
			vkCmdBindVertexBuffers(m_CommandBuffers[Count], 0, 1, vertexBuffers, offsets);
			vkCmdBindIndexBuffer(m_CommandBuffers[Count], Obj->GetIndex().m_Buffer, 0, VK_INDEX_TYPE_UINT16);
			//vkCmdBindDescriptorSets(m_CommandBuffers[Count], VK_PIPELINE_BIND_POINT_GRAPHICS, m_PipelineLayout, 0, 1, &m_DescriptorSets[Count], 0, nullptr);
		}
...
void Graphic_Vulkan::DrawFrame()
{
	...
	for (GV_Obj* Obj : m_Object)
	{
		...
		case BUFFER_TYPE::SPRITE_BUFFER:
			{
				Sprite_Obj* Spr = dynamic_cast<Sprite_Obj*>(Obj);
				updateUniformBuffer(imageIndex, *Spr);
			}
			break;
		case BUFFER_TYPE::RECTANGLE_BUFFER:
			{
...
void Graphic_Vulkan::DrawFrame(std::chrono::steady_clock::time_point _deltaTime)
{
	vkWaitForFences(m_Device, 1, &m_InfFlightFence[m_CurrentFrame], VK_TRUE, UINT64_MAX);
...
void Graphic_Vulkan::createVertexBuffer(std::vector<Vertex> _vertex, VkBuffer* _vertexBuffer, VkDeviceMemory* _gpuVertexMemroy)
{
	VkDeviceSize BufferSize = sizeof(_vertex[0]) * _vertex.size();
	VkBuffer StagingBuffer = VK_NULL_HANDLE;
	VkDeviceMemory stagingBufferMemory = VK_NULL_HANDLE;
	createBuffer(BufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, StagingBuffer, stagingBufferMemory);
	...
	void* Data;
	vkMapMemory(m_Device, stagingBufferMemory, 0, BufferSize, 0, &Data);
	memcpy(Data, _vertex.data(), (size_t)BufferSize);
	vkUnmapMemory(m_Device, stagingBufferMemory);

	createBuffer(BufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *_vertexBuffer, *_gpuVertexMemroy);
...
GV_Buffer Graphic_Vulkan::createVertexBuffer(std::vector<Vertex> _vertex)
{
	VkDeviceSize BufferSize = sizeof(_vertex[0]) * _vertex.size();
	VkBuffer ReturnBuffer = VK_NULL_HANDLE;
	VkBuffer StagingBuffer = VK_NULL_HANDLE;
	VkDeviceMemory ReturnBufferMemory = VK_NULL_HANDLE;
	VkDeviceMemory stagingBufferMemory = VK_NULL_HANDLE;
	createBuffer(BufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, StagingBuffer, stagingBufferMemory);
	...
	createBuffer(BufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, ReturnBuffer, ReturnBufferMemory);

	copyBuffer(StagingBuffer, ReturnBuffer, BufferSize);

	vkDestroyBuffer(m_Device, StagingBuffer, nullptr);
	vkFreeMemory(m_Device, stagingBufferMemory, nullptr);

	return GV_Buffer(ReturnBuffer, ReturnBufferMemory);
}
...
void Graphic_Vulkan::createIndexBuffer(std::vector<uint16_t> _indices, VkBuffer* _indicesBuffer, VkDeviceMemory* _gpuIndicesMemroy)
{
	...
	createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *_indicesBuffer, *_gpuIndicesMemroy);
...
GV_Buffer Graphic_Vulkan::createIndexBuffer(std::vector<uint16_t> _indices)
{
	VkDeviceSize bufferSize = sizeof(_indices[0]) * _indices.size();
	VkBuffer ReturnBuffer = VK_NULL_HANDLE;
	VkBuffer StagingBuffer = VK_NULL_HANDLE;
...
void Graphic_Vulkan::createUniformBuffers(GV_Obj& _Obj)
{
	VkDeviceSize bufferSize = sizeof(UniformBufferObject);

	_Obj.ResizeUniform(m_SwapChainImages.size());

	for (size_t Count = 0; m_SwapChainImages.size() > Count; ++Count)
	{
		GV_Buffer Uniform;
		createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, Uniform.m_Buffer, Uniform.m_BufferMemory);
		_Obj.InsertUniform(static_cast<int>(Count), Uniform);
	}
}
...
void Graphic_Vulkan::updateUniformBuffer(uint32_t _currentImage, GV_Obj& _Obj)
{
	static std::chrono::steady_clock::time_point startTime = std::chrono::high_resolution_clock::now();
	...
	///카메라 객체가 view, proj 행렬을 제공할것
	UniformBufferObject ubo = {};
	ubo.world = _Obj.MatWorld();
	//ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	//ubo.proj = glm::perspective(glm::radians(45.0f), m_SwapChainExtent.width / (float)m_SwapChainExtent.height, 0.1f, 10.0f);
	ubo.view = m_Camera[0]->GetView();
	ubo.proj = m_Camera[0]->GetProj();
	ubo.proj[1][1] *= -1;

	void* Data;
	vkMapMemory(m_Device, _Obj.Uniform((int)_currentImage).m_BufferMemory, 0, sizeof(ubo), 0, &Data);
	memcpy(Data, &ubo, sizeof(ubo));
	vkUnmapMemory(m_Device, _Obj.Uniform((int)_currentImage).m_BufferMemory);
}
...
void Graphic_Vulkan::createDescriptorSets(size_t _ObjCount, Sprite_Obj& _Obj)
{
	std::vector<VkDescriptorSetLayout> layouts(m_SwapChainImages.size(), descriptorSetLayout);
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	...
	{
		VkDescriptorBufferInfo bufferInfo = {};
		bufferInfo.buffer = _Obj.Uniform(Count).m_Buffer;
		bufferInfo.offset = 0;
		bufferInfo.range = sizeof(UniformBufferObject);
		...
		VkDescriptorImageInfo imageInfo = {};
		imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo.imageView = _Obj.Sprite(_ObjCount, 0).m_ImageView;
		imageInfo.sampler = _Obj.Sprite(_ObjCount, 0).m_Sampler;
...
void Graphic_Vulkan::createTextureImage()
{
	...
	int texWidth = 0, texHeight = 0, texChannels = 0;
	stbi_uc* pixels = stbi_load("textures/EavanKimTrash.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	VkDeviceSize ImageSize = texWidth * texHeight * 4;
...
void Graphic_Vulkan::CreateTextureSampler(uint64_t& _Sampler)
{
	VkSamplerCreateInfo samplerInfo = {};
	samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	samplerInfo.magFilter = VK_FILTER_LINEAR;
	...
	if (vkCreateSampler(m_Device, &samplerInfo, nullptr, &_Sampler) != VK_SUCCESS)
		throw std::runtime_error("failed to create texture sampler!");
}
///
